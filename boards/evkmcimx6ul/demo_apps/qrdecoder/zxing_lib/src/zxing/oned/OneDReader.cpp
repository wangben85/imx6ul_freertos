...
Ref<Result> OneDReader::decode(Ref<BitMatrix> image, DecodeHints hints) {
  try {
...
Ref<Result> OneDReader::doDecode(Ref<BitMatrix> image, DecodeHints hints) {
  ...
  int middle = height / 2;
  bool tryHarder = hints.getTryHarder();
  int rowStep = std::max(1, height >> (tryHarder ? 8 : 6));
  using namespace std;
  ...
  for (int x = 0; x < maxLines; x++) {
    ...
    try {
      row = image->getRow(rowNumber, row);
