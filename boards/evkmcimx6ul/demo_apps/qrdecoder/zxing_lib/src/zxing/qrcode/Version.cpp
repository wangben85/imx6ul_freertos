...
ECBlocks::~ECBlocks() {
  for (std::size_t i = 0; i < ecBlocks_.size(); i++) {
    delete ecBlocks_[i];
  }
}
...
Version::Version(int versionNumber, vector<int> *alignmentPatternCenters, ECBlocks *ecBlocks1, ECBlocks *ecBlocks2,
  ...
  for (std::size_t i = 0; i < ecbArray.size(); i++) {
...
Ref<BitMatrix> Version::buildFunctionPattern() {
  ...
  // Alignment patterns
  std::size_t max = alignmentPatternCenters_.size();
  for (std::size_t x = 0; x < max; x++) {
    int i = alignmentPatternCenters_[x] - 2;
    for (std::size_t y = 0; y < max; y++) {
...
static vector<int> *intArray(std::size_t n...) {
  std::va_list ap;
  va_start(ap, n);
