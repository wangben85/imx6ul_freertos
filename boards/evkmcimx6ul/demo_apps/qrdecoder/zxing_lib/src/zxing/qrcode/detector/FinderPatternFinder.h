...
protected:
  ...
  float crossCheckVertical(std::size_t startI, std::size_t centerJ, int maxCount, int originalStateCountTotal);
  float crossCheckHorizontal(std::size_t startJ, std::size_t centerI, int maxCount, int originalStateCountTotal);

  /** stateCount must be int[5] */
  bool handlePossibleCenter(int* stateCount, std::size_t i, std::size_t j);
  int findRowSkip();
