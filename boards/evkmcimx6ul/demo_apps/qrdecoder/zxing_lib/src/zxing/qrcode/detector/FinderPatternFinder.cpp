...
float FinderPatternFinder::crossCheckVertical(std::size_t startI, std::size_t centerJ, int maxCount, int originalStateCountTotal) {
...
float FinderPatternFinder::crossCheckHorizontal(std::size_t startJ, std::size_t centerI, int maxCount,
...
bool FinderPatternFinder::handlePossibleCenter(int* stateCount, std::size_t i, std::size_t j) {
  int stateCountTotal = stateCount[0] + stateCount[1] + stateCount[2] + stateCount[3] + stateCount[4];
  float centerJ = centerFromEnd(stateCount, j);
  float centerI = crossCheckVertical(i, (std::size_t)centerJ, stateCount[2], stateCountTotal);
  if (!isnan(centerI)) {
    // Re-cross check
    centerJ = crossCheckHorizontal((std::size_t)centerJ, (std::size_t)centerI, stateCount[2], stateCountTotal);
    if (!isnan(centerJ)) {
      float estimatedModuleSize = (float)stateCountTotal / 7.0f;
      bool found = false;
      std::size_t max = possibleCenters_.size();
...
vector< Ref<FinderPattern> > FinderPatternFinder::selectBestPatterns() {
  std::size_t startSize = possibleCenters_.size();
  ...
  if (possibleCenters_.size() > 3) {
    ...
    float totalModuleSize = 0.0f;
    for (std::size_t i = 0; i < possibleCenters_.size(); i++) {
      float size = possibleCenters_[i]->getEstimatedModuleSize();
      totalModuleSize += size;
    }
...
Ref<FinderPatternInfo> FinderPatternFinder::find(DecodeHints const& hints) {
  ...
  std::size_t maxI = image_->getHeight();
  std::size_t maxJ = image_->getWidth();
  ...
  for (std::size_t i = iSkip - 1; i < maxI && !done; i += iSkip) {
