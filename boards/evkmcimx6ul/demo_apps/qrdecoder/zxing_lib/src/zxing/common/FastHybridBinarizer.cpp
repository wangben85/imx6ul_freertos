...
void FastHybridBinarizer::writeBlockRow(uint8_t *blockRow)
{
    ...
    while ((blockRowsRead < numBlockRows) && (blockRowsWritten >= 5) && ((blockRowsRead <= (blockRowsWritten - 3)) || (blockRowsRead >= (numBlockRows - 2))))
    {
...
//    int sum, max, min;
//    int yoffset = blockRowsWritten*8;
//    for(int i=0; i<width_/8; i++)
//    {
//        int xoffset = i*8;
//        int location = yoffset * width_ + xoffset;
//        thresholdBlock(&(matrix[location >> 3]), &(blockRow[xoffset]), width_, 192);
//    }
//    
//    blockRowsWritten++;
}
