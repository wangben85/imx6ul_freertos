...
Ref<Result> MultiFormatReader::decode(Ref<BitMatrix> image) {
  setHints(DecodeHints::DEFAULT_HINT);
  return decodeInternal(image);
}

Ref<Result> MultiFormatReader::decode(Ref<BitMatrix> image, DecodeHints hints) {
  setHints(hints);
  return decodeInternal(image);
}

Ref<Result> MultiFormatReader::decodeWithState(Ref<BitMatrix> image) {
...
Ref<Result> MultiFormatReader::decodeInternal(Ref<BitMatrix> image) {
